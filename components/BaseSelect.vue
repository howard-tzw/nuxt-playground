...
const noDropBackgroundColor = computed(() => (attrs['no-drop'] ? '#f7f7f7' : '#fff'))
...
<template>
	<div>
		...
		>
			...
			<template #no-options="{ search, searching, loading }"> 暫無選項 </template>
		</v-select>
	</div>
</template>
...
:deep(.vs__dropdown-toggle .vs__selected-options .vs__selected::-webkit-scrollbar) {
	display: none;
}
